...
"""Hasil di atas menunjukkan bahwa terdapat sebanyak 3.854 data duplikat pada dataset ini."""

df[df.duplicated()]

"""Hasil di atas menunjukkan data duplikat yang ada pada dataset yang digunakan."""

df.describe()

"""**Insight:**

- Hasil di atas menunjukkan deskripsi statistik untuk masing-masing fitur. Rata-rata usia (age) adalah sekitar 41.88 tahun, dengan rentang antara 0.08 hingga 80 tahun, menunjukkan variasi usia yang luas.
...
- Untuk HbA1c_level, nilai rata-ratanya adalah 5.52, dengan 75% data di bawah 6.2, mendekati ambang diagnosis diabetes.
- Sementara itu, kadar glukosa darah (blood_glucose_level) rata-ratanya adalah 138, yang berada pada ambang batas toleransi normal.
- Target diabetes memiliki nilai rata-rata 0.085, yang berarti sekitar 8.5% dari total populasi adalah penderita diabetes.
...
### Exploratory Data Analysis - Memeriksa Missing Value dan Outliers
...
Dari hasil di atas, tidak ditemukan missing value pada dataset ini.
...
"""**Insight:**
<br>Pada visualisasi bloxpot pada fitur-fitur numerik di atas, dapat kita lihat bahwa pada beberapa fitur numerik yang memiliki outliers, diantaranya yang perlu ditangani adalah bmi, HbA1c_level, dan blood_glucose_level.
...
Terdapat 6 kategori pada fitur gender, secara berurutan dari jumlahnya yang paling banyak yaitu: No Info, never, former, current, not current, dan ever. Mayoritas data berasal dari individu dengan riwayat merokok yang tidak diketahui atau tidak diisi (kategori No Info) sebanyak 35.8%, yang berarti terdapat kekosongan informasi yang cukup besar dalam fitur ini. Selain itu, proporsi terbesar berikutnya adalah yang tidak pernah merokok (kategori never) sebesar 35.1%. Kategori lainnya menunjukkan bahwa sebagian kecil populasi merupakan perokok aktif atau mantan perokok.
...
Pada heatmap korelasi di atas, fitur `HbA1c_level` dan `blood_glucose_level` memiliki skor korelasi tertinggi dengan fitur target diabetes, dengan nilai masing-masing yaitu 0.4 dan 0.42, diikuti oleh `age` dengan nilai 0.26, serta `bmi, hypertension dan heart_disease` dengan nilai masing-masing 0.21, 0.2 dan 0.17. Artinya, fitur `HbA1c_level, blood_glucose_level dan age` memiliki korelasi yang cukup berarti dengan diabetes.
...
"""Output di atas menunjukkan bahwa dataset sekarang telah bersih dari data duplikat dan memiliki 96.146 sampel tersisa.
...
"""Output di atas menunjukkan bahwa dataset sekarang telah bersih dari outliers dan memiliki 88.195 sampel tersisa.
